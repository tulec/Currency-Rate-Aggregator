...
Для race detector требуется поддерживаемая 64-битная платформа, cgo и C-компилятор. Текущее окружение `windows/386` с `CGO_ENABLED=0` его не поддерживает.
...
Подробная история этапов находится в [ROADMAP.md](ROADMAP.md).
